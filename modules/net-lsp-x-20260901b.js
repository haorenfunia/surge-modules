    ...
    if (ENTRANCE_IP && ENTRANCE_IP !== PROXY_IP) {
      ...
      if (!ENTRANCE_INFO2) {
        ...
        const entranceLocation = String(ENTRANCE_INFO1 || '')
          .split(/\n/)
          .find(i => /^位置\s*[:：]/.test(i))
        const entranceMetadata = String($.lodash_get(entranceLegacyInfo, 'PROXY_INFO') || '')
          .split(/\n/)
...
// 通知
// EVENT_NOTIFY: 网络变化事件通知开关, 1 为开启(默认), 0 为静默(仍查询并更新缓存, 不发通知)
async function notify(title, subt, desc, opts) {
  const eventNotify = $.lodash_get(arg, 'EVENT_NOTIFY')
  const eventNotifyEnabled = !(eventNotify == 0 || eventNotify === '0' || eventNotify === false)
  if (($.lodash_get(arg, 'TYPE') === 'EVENT' && eventNotifyEnabled) || $.lodash_get(arg, 'notify') == 1) {
